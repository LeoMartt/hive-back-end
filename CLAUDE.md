...
Campos adicionais fixos (antes previstos como customizáveis por projeto via `CustomField` — descartado em 2026-08-26): Área, Sistema, Observações, Resultado Esperado, WBS, Transação, outros. Viram campos fixos direto no model `Activity`, iguais pra todo projeto.
...
Estado do frontend (04 telas reais implementadas com dado mockado, sem API real ainda): lista de Projetos, Dashboard do projeto, lista de Atividades, lista de Issues. Login SSO via Entra ID (MSAL) e `ProtectedRoute` já funcionam ponta a ponta no front — falta só o backend validar o Access Token. Auth real configurada em `hive-front-end/src/config/authConfig.ts` (tenant/client ID do Entra ID da FUMEP/EEP, scopes `User.Read` + `User.ReadBasic.All`).
...
1. **Hierarquia de Atividade fixada como 2 níveis, não configurável.** `Activity` (`hive-front-end/src/types/activity.ts`) tem campos fixos `module`/`process` (2 níveis + Atividade como folha), em vez de um array genérico correspondente a `HierarchyLevel` (nomes configuráveis por projeto, conforme já planejado em `apps/projects`). Precisa decidir: o backend expõe `module`/`process` como nomenclatura fixa (e o front já assumiu certo) ou o front vai precisar de refatoração para consumir níveis dinâmicos quando a API real chegar?
...
6. **Limiares de Aging/Risco de Issue hardcoded no front** (`alerta: 2 dias, risco: 6 dias`, fixos para o modo UAT — `hive-front-end/src/utils/issueIndicators.ts`), sem equivalente nas RN01–RN44 aqui documentadas. A spec do front já assume que isso deveria ser configurável por projeto/modo via uma futura tela "Papéis & Config" — se confirmado, o backend precisa de um lugar para esse limiar (com `CustomField` descartado, o candidato passa a ser campo fixo de configuração em `Project`, não mais um schema genérico).
